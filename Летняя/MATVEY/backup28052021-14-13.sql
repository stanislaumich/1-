...
CREATE TABLE `a` (
	`fio` VARCHAR(250) NOT NULL COLLATE 'utf8mb4_0900_ai_ci',
	`data` DATE NOT NULL
) ENGINE=MyISAM;
...
-- Создание временной таблицы для обработки ошибок зависимостей представлений
CREATE TABLE `b` (
	`marka` VARCHAR(45) NOT NULL COLLATE 'utf8mb4_0900_ai_ci',
...
-- Создание временной таблицы для обработки ошибок зависимостей представлений
CREATE TABLE `c` (
	`fio` VARCHAR(500) NOT NULL COLLATE 'utf8mb4_0900_ai_ci',
	`uradr` VARCHAR(500) NOT NULL COLLATE 'utf8mb4_0900_ai_ci'
) ENGINE=MyISAM;
...
CREATE TABLE `d` (
	...
	`numchet` VARCHAR(50) NOT NULL COLLATE 'utf8mb4_0900_ai_ci'
) ENGINE=MyISAM;
...
CREATE TABLE `e` (
	`fio` VARCHAR(250) NOT NULL COLLATE 'utf8mb4_0900_ai_ci',
	`dolg` VARCHAR(500) NULL COLLATE 'utf8mb4_0900_ai_ci'
) ENGINE=MyISAM;
...
INSERT INTO `kadry` (`idkadry`, `fio`, `dolg`, `obraz`, `oklad`, `data`) VALUES
...
INSERT INTO `klient` (`idklient`, `fio`, `adr`, `idschet`) VALUES
...
INSERT INTO `transport` (`idtransport`, `nomer`, `marka`, `gruz`, `god`, `vladelec`, `data`, `nsklad`) VALUES
...
INSERT INTO `zakaz` (`idzakaz`, `data`, `id_client`, `id_tovar`, `kolvo`, `cena`, `summa`, `opl`) VALUES
	(1, '2021-05-19', 2, 2, 3, 2300, 6900, NULL),
	(2, '2021-05-19', 4, 3, 1, 520, 520, NULL),
	(3, '2021-05-22', 5, 3, 1, 5890, 5890, NULL),
	(4, '2019-03-16', 6, 6, 1, 789, 678, NULL);
...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `a` AS select `kadry`.`fio` AS `fio`,`kadry`.`data` AS `data` from `kadry` where (`kadry`.`data` < '2021-01-01');

-- Дамп структуры для представление matvey.b
DROP VIEW IF EXISTS `b`;
-- Удаление временной таблицы и создание окончательной структуры представления
DROP TABLE IF EXISTS `b`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `b` AS select `transport`.`marka` AS `marka`,`transport`.`gruz` AS `gruz`,`transport`.`god` AS `god`,`transport`.`data` AS `DATA` from `transport` where ((`transport`.`data` < '2015-01-01') and (`transport`.`gruz` = 1000));

-- Дамп структуры для представление matvey.c
DROP VIEW IF EXISTS `c`;
-- Удаление временной таблицы и создание окончательной структуры представления
DROP TABLE IF EXISTS `c`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `c` AS select `p`.`fio` AS `fio`,`p`.`uradr` AS `uradr` from (`tovar` `t` join `postav` `p`) where ((`p`.`idpostav` = `t`.`idpostav`) and (`t`.`sort` = 1) and (`t`.`cena` = 500) and (`t`.`naimen_tovara` = 'Телефон'));

-- Дамп структуры для представление matvey.d
DROP VIEW IF EXISTS `d`;
-- Удаление временной таблицы и создание окончательной структуры представления
DROP TABLE IF EXISTS `d`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `d` AS select `p`.`fio` AS `fio`,`s`.`bank` AS `bank`,`s`.`numchet` AS `numchet` from (`schet` `s` join `postav` `p`) where ((`p`.`fio` = 'Название') and (`p`.`idschet` = `s`.`idschet`));

-- Дамп структуры для представление matvey.e
DROP VIEW IF EXISTS `e`;
-- Удаление временной таблицы и создание окончательной структуры представления
DROP TABLE IF EXISTS `e`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `e` AS select `kadry`.`fio` AS `fio`,`kadry`.`dolg` AS `dolg` from (`kadry` join `transport` on((`kadry`.`fio` = `transport`.`vladelec`)));
